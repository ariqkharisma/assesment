...
export default function Home() {
  const { loading, isLoggedIn, userInfo, logout, fetchData, data } = useGlobalContext();
      ...
      {loading ? (
        ...
        isLoggedIn && (
          ...
          >
            ...
            <button
              onClick={logout}
              className="button bg-red-400 hover:bg-red-500 py-2 px-4 rounded-lg my-5 text-white"
            >
              Logout
            </button>
            <div className="w-full flex justify-between mt-10 mb-8">
              ...
              {userInfo?.role === "admin" && (
                <button onClick={() => router.push("/users/add")} className="button py-2 px-4 bg-blue-300 hover:bg-blue-400 rounded-lg">
                  Add User
                </button>
              )}
            </div>
